...
`STATE_CHANGED_ABORT` 特別要強調一點，它不是故障而是防護生效了。四個排程共用一份
狀態，補跑時可能兩輪重疊，腳本寧可讓後到的那輪整個放棄，也不要讓它把先到那輪的
...
| 佇列 | 出現在 `begin` 的哪一欄 | 什麼會讓它離開 |
|---|---|---|
| 待通知 | `notifyNow` | 只有 `notifiedIds`，也就是 toast 確定送出 |
| 待判 | `judgeNow` / `judgeOverdue` | `judgedIds`，或把它列進 `important`（判為重要就轉進待通知佇列了） |
| 待辦 | 不在 `begin` 裡，在待辦清單上 | 只有使用者在 GUI 勾選完成，你永遠不要試圖移除 |

所以 `judgedIds` 是**待判佇列**的主要出路，但不是三個佇列共通的唯一出路。
...
- `archiveBlocked` / `restoreBlocked` / `promoteBlocked` / `sweepBlocked` —
  四個都是使用者在 GUI 上按的東西沒辦成。非空字串就是原因，空字串代表沒事。
  **一個都不能漏看**，全部照同一種方式處理，見下面那張表

| 欄位 | 使用者按了什麼 | 他會看到的症狀 |
|---|---|---|
| `archiveBlocked` | 勾選完成 | 勾過的項目一直不消失 |
| `restoreBlocked` | 在已封存按「復原」 | 按了卻沒回到待辦清單 |
| `promoteBlocked` | 在重要事項按「加到待辦」 | 按了卻沒出現在「這次新增」 |
| `sweepBlocked` | 沒按（是腳本要把沒處理的重要事項歸檔） | 重要事項那一區清不掉、越積越多 |

**這四種都不會自己好**，而且使用者會以為是自己沒按到。非空字串就把那句話接在
推播末端告訴他，**即使這輪沒有重要信也要推**。`archiveBlocked` 的典型值是
`tick file unreadable` / `archive unreadable` / `archive not writable`，其餘三個
形狀類似。不要為它們單獨發第二則推播，接在同一則末端就好
...
但那個元件同時有四個排程程序在跑，所以它們另外各自靠 rev 檢查擋。
...
幾乎同時觸發時會互相碰撞，腳本用 rev 檢查擋下來，輸的那輪收到
`STATE_CHANGED_ABORT` 整輪放棄。那一輪掃過的區間沒有退休，下一輪會重掃，
所以常見結果是白跑一輪加上下一輪重複通知，方向是安全的。
