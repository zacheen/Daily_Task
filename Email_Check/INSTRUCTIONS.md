...
這是四個排程任務（06:25 / 11:40 / 16:55 / 22:10）共用的唯一指令來源。
...
理由是排隊本身有代價。四個時段間隔 5.25 到 8.25 小時，等兩輪就是十幾個小時，
...
目前就是使用者在 Gmail 上自己掛的 `ignore` 標籤。**掛了標籤就是使用者已經看過
並決定不要再被提醒**，所以那封信連抓都不用抓。這比 `excludedSenders` 強，
因為標籤可以只掛在某個寄件人的其中一封信上，而那個寄件人別的信照樣會進來。
...
- **雇主自己辦的招募活動邀約，即使還沒報名過也算。** 公司自己的 info session、
  ...
  **這類一樣要先查日曆。** `FOUND` 代表使用者早就報名並排進行程了，
  報名與出席都不用再建待辦，結案。`NOT_FOUND` 與 `UNCHECKED` 照樣建待辦。
...
- 純資訊性通知、單筆交易收據、已完成的交易紀錄。
  ...
  **券商的成交通知與交易確認書屬於這一類，憑主旨就排除得掉，不要讀內文。**
  實例是 `Schwab Alerts` 寄的 `Your trade was executed...` 與 `Schwab eConfirms...`，
  內容是已成交的單筆買賣明細，動作在信寄出之前就完成了。
  這是「銀行寄來的信要讀內文」那一條的例外，因為主旨本身已經講明了信的種類。
  分界沒有變，月結對帳單要核對，單筆成交不用核對，所以同一個寄件人寄來
  對帳單或帳務異常通知時照樣走主標準。

  這一類多半在抓進來之前就被 `ignore` 標籤濾掉了，見第 3 步。這條規則是
  萬一漏網的保險，不是主要機制。**不要把寄件人加進 `excludedSenders`**，
  券商的成交通知跟對帳單、帳務異常通知共用同一個 `donotreply@` 地址，
  整個地址濾掉會讓後兩種一起消失，方向跟最高原則相反。
...
2. 學校、政府、銀行、保險、房東、學貸寄來的信，只要還沒排除必辦事項。
   券商的成交通知與交易確認書是這一條的例外，見上面「不算有下一步動作的例子」
...
06:25 那一輪即使有也不會開，因為那時候彈一個瀏覽器視窗出來只會吵醒使用者。
