...
| `judgeNow` | 這輪可以用預算判定的舊欠帳 |
| `bodyBudgetTotal` / `bodyBudgetForDebt` | 內文預算總額與其中分給舊欠帳的額度 |
...
**`judgeNow`** 用 `bodyBudgetForDebt` 的額度去判，剩下的留給這輪的新信。
...
要先確認裡面沒有必辦事項才能放掉。必要時動用內文預算，
snippet 看不完就讀內文，這是預算的優先用途之一
...
- **已報名或已答應的活動，只要有具體日期時間。** 例如已報名的工作坊、說明會、
  ...
  **這類信常常在時間地點下面再夾一個動作，例如填報名表、登記票券、繳費、
  ...
  判成已報名活動之後要讀內文把夾帶的動作撈出來，見下面「讀內文的預算怎麼花」
...
mcp__gmail__get_email_body。額度照 begin 給的 bodyBudgetTotal 與 bodyBudgetForDebt。

預算優先序，從高到低
1. judgeNow 裡的舊欠帳，受 bodyBudgetForDebt 限制
...
不要把預算花在純商業行銷或平台 job alert 上。

**預算用完還判斷不出來的，不可以當成不重要。**
...
```json
{
  ...
  "defer": [{"id": "...", "from": "...", "subject": "...", "snippet": "..."}],
  "todos": [{"id": "...", "from": "...", "subject": "...",
...
腳本會原封不動留到下一輪。這是刻意的，因為 `begin` 只給你看預算切片，
排在切片外的欠帳你根本看不到，若把「沒回報」當成「已處理」就會靜默漏掉它們。
...
不要等到最後。** `defer` 特別容易被忽略 —— 一封因內文預算不足而還沒判定的信
...
`defer` 放預算不足還沒判定完的信。存 `from` / `subject` / `snippet` 而不是只存 id，
同樣是因為之後拿不回上下文。
...
永遠無法用 id 去重、無法勾選完成、也不會消耗內文預算，就這樣卡在佇列裡。
所以真正的重點是**預防**，見下面那條。
